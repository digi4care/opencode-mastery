...
echo ""
echo "  📚 Skills"
echo "  • OpenCode, frontend, backend, testing, analysis, GSD validation"
echo ""
echo "  🔌 Plugins"
echo "  • TypeScript runtime tools for session/debug/TDD/flow/repo/skill/GSD validation"
echo ""
echo "  ⚙️  Features"
echo "  • Modular architecture with centralized tool registry"
echo ""
echo "  🤖 Agents:"
...
if [ "$PLUGINS_CAN_BUILD" = true ]; then
    cd "$TEMP_DIR"
    echo "  Running: bun run scripts/deploy.ts"
    if timeout 120 bun run scripts/deploy.ts; then
        echo "  ✓ Plugin build complete"
    else
        echo "  ⚠️  Build failed (or timed out), plugins will use TypeScript source"
    fi
fi
