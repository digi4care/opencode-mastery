...
```bash
# Stap 1: Installatie (download + copy + dependency bootstrap)
./install.sh                 # Van GitHub
./install.sh -y              # Zonder bevestiging

# Stap 2: Plugins compilen (aanbevolen, in project root)
bun run deploy               # Compileert TypeScript plugins naar ~/.config/opencode/plugin/*/index.js
...
2. `./install.sh` probeert plugin build in temp-dir (`bun run scripts/deploy.ts`)
3. `bun run deploy` in project root is de betrouwbare developer stap om plugin JS artifacts te garanderen
4. `./uninstall.sh` verwijdert alleen beheerde bestanden uit `~/.config/opencode/`
